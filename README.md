...
compiler options:
    cmake PATH -DCMAKE_BUILD_TYPE=Debug (for profiling)
    cmake PATH -DCMAKE_BUILD_TYPE=RelWithDebug (for profiling and with some optimization)
    cmake PATH -DCMAKE_BUILD_TYPE=Release (with better perf on CPU, not optimizinig the perf on CPU)
